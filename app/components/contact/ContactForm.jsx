  ...
  return (
    <div className="relative w-full group">
      <div className="flex items-center gap-3 mb-2">
        <div
          className={`p-1.5 rounded-lg border transition-all duration-300 ${isFocused ? "bg-color-1/10 border-color-1 shadow-[0_0_15px_rgba(59,130,246,0.3)]" : "bg-n-8/50 border-n-6/30"}`}
        >
          {React.cloneElement(icon, {
            className: `w-4 h-4 transition-colors ${isFocused ? "text-color-1" : "text-n-4"}`,
          })}
        </div>
        <label
          className={`text-[10px] font-mono uppercase tracking-[0.2em] transition-colors ${isFocused ? "text-color-1" : "text-n-4"}`}
        >
      ...
      <div className="relative">
        {isTextArea ? (
          <textarea
            ...
            required={required}
            rows="4"
            className="w-full bg-n-8/30 border border-n-2/30 rounded-xl px-4 py-4 text-n-1 dark:text-white focus:outline-none transition-all placeholder:text-n-4/30 resize-none group-hover:border-n-5/50 focus:border-color-1/50"
          />
        ) : (
          <input
            type={type}
            ...
            placeholder={placeholder}
            required={required}
            className="w-full bg-n-8 border border-n-4/40 dark:border-n-5/30 rounded-xl px-4 py-4 text-n-1 dark:text-white focus:outline-none transition-all placeholder:text-n-4/30 group-hover:border-n-4/50 focus:border-color-1/50"
          />
        )}
      ...
      <form onSubmit={handleSubmit} className="space-y-12">
        ...
        <div className="space-y-8">
          <div className="flex items-center gap-4">
            <span className="text-[10px] font-mono bg-n-7 dark:bg-white/5 px-2 py-1 rounded border border-n-6/30 text-color-1">
              MOD_01
            </span>
            <div className="h-[1px] flex-1 bg-gradient-to-r from-n-6/30 to-transparent" />
            <h4 className="text-[10px] font-mono uppercase tracking-[0.3em] text-n-4">
              Identity Matrix
            </h4>
          </div>
        ...
        <div className="space-y-8">
          <div className="flex items-center gap-4">
            <span className="text-[10px] font-mono bg-n-7 dark:bg-white/5 px-2 py-1 rounded border border-n-6/30 text-color-2">
              MOD_02
            </span>
            <div className="h-[1px] flex-1 bg-gradient-to-r from-n-6/30 to-transparent" />
            <h4 className="text-[10px] font-mono uppercase tracking-[0.3em] text-n-4">
                ...
                className={`flex flex-col items-center justify-center p-4 rounded-2xl border transition-all duration-300 gap-3 group/chip
                  ${
                    formData.projectType.includes(type.id)
                      ...
                      : "bg-n-8/30 border-n-4/40 dark:border-n-6/30 text-n-4 hover:border-n-5/50"
                  }
                `}
              >
                <div
                  className={`p-2 rounded-xl border transition-all ${formData.projectType.includes(type.id) ? "bg-color-1 border-white/20 text-white" : "bg-n-6 dark:bg-n-7 border-n-6/30"}`}
                >
                  {type.icon}
                </div>
        ...
        <div className="space-y-8">
          <div className="flex items-center gap-4">
            <span className="text-[10px] font-mono bg-n-7 dark:bg-white/5 px-2 py-1 rounded border border-n-6/30 text-color-3">
              MOD_03
            </span>
            <div className="h-[1px] flex-1 bg-gradient-to-r from-n-6/30 to-transparent" />
            <h4 className="text-[10px] font-mono uppercase tracking-[0.3em] text-n-4">
