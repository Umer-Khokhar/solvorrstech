...
const SubServices = ({ subServices }) => {
  ...
  return (
    <section
      id="sub-services"
      className="relative pt-[5rem] py-12 xl:py-24 px-4 sm:px-6 lg:px-8 bg-n-8 dark:bg-n-8 overflow-hidden"
    >
      <TechBackground />
      {/* <SubtleBackground /> */}
                ...
                <div key={index} className="relative">
                  ...
                  >
                    <div className="flex items-center gap-6">
                      <span
                        className={`font-mono text-xs transition-colors duration-300 ${activeService === index ? "text-color-1" : "text-n-4"}`}
                      >
                        0{index + 1}
                      </span>
                      <span
                        className={`text-xl lg:text-2xl font-medium tracking-tight transition-transform duration-300 ${activeService === index ? "translate-x-1" : "group-hover:translate-x-1"}`}
                      >
                        {service.title}
                      </span>
                    </div>
                    ...
                    <ArrowUpRight
                      className={`w-5 h-5 transition-all duration-300 ${activeService === index ? "opacity-100 text-color-1" : "opacity-0 -translate-y-1 translate-x-1"}`}
                    />
                  </button>
            ...
            <div className="h-full relative ">
              ...
              <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <div className="max-w-2xl">
                      <div className="flex items-center gap-4 mb-8">
                        <div className="w-12 h-12 rounded-2xl bg-n-7 dark:bg-white/5 flex items-center justify-center border border-n-6/50 dark:border-white/10">
                          {(() => {
                            const Icon =
                              icons[subServices[activeService]?.icon] ||
                              Sparkles;
                            return (
                              <Icon
                                className="w-6 h-6 text-n-1 dark:text-white"
                                strokeWidth={1.5}
                              />
                            );
                          })()}
                      ...
                      <h3
                        className="text-4xl lg:text-5xl font-semibold text-n-1 dark:text-white mb-6 tracking-tight leading-[1.1]"
                        style={{ fontFamily: "var(--font-clash-grotesk)" }}
                      >
                  ...
                  {subServices[activeService]?.features &&
                    subServices[activeService].features.length > 0 && (
                      <div className="mb-12">
                        <div className="h-[1px] w-full bg-n-6/30 dark:bg-white/5 mb-8" />
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
                          {subServices[activeService].features.map(
                            (feature, idx) => (
                              <div
                                key={idx}
                                className="flex items-start gap-4 group/feat"
                              >
                                <span className="text-[10px] font-mono text-color-1 mt-1 opacity-60 group-hover/feat:opacity-100 transition-opacity">
                                  0{idx + 1}
                                </span>
