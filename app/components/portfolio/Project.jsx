  ...
  return (
    ...
    >
        ...
        <div className="relative md:w-1/2">
            <motion.h4 variants={itemVariants} className='h5 lg:h4 max-w-xl mb-4 md:mb-8 text-n-1'>{title}</motion.h4>
            <motion.p variants={itemVariants} className='body-2 text-n-3 mb-6 leading-relaxed'>{desc}</motion.p>
            
            <motion.div variants={itemVariants} className="technologies flex items-center gap-2 flex-wrap mb-8">
                <h5 className='h6 text-n-1 mr-2'>Technologies:</h5>
                {tech.map((item, index) => (
                    <span key={index} className='px-4 py-1.5 bg-n-6/50 backdrop-blur-sm rounded-full body-2 font-code text-n-3 border border-n-1/10 hover:border-n-1/40 hover:text-n-1 transition-all duration-300'>
                        {item}
                    </span>
                ))}
            </motion.div>
                ...
                <ul className='flex flex-col gap-6'>
                    {features.map((item, index) => (
                        ...
                        >
                            ...
                            <p className="text-n-2 group-hover:text-n-1 transition-colors duration-300">{item}</p>
                        </motion.li>
                    ))}
                </ul>
            </div>
